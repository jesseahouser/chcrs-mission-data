  ...
  switch(api) {
    case 'Geocoding':
      for (i = 0; i < data.length; i++) {
        if (data[i] != 'No Data') {
          var latitudeCell = sheet.getRange(
            table.firstDataRow + i,
            table.latitudeColumn)
          var longitudeCell = sheet.getRange(
            table.firstDataRow + i,
            table.longitudeColumn)
          var geographicCoordinatesCell = sheet.getRange(
            table.firstDataRow + i,
            table.geographicCoordinatesColumn)
          var addressFormattedCell = sheet.getRange(
            table.firstDataRow + i,
            table.addressFormattedColumn)
          var countyCell = sheet.getRange(
            table.firstDataRow + i,
            table.countyColumn)
          var stateCell = sheet.getRange(
            table.firstDataRow + i,
            table.stateColumn)
          var countyStateAbbreviatedCell = sheet.getRange(
            table.firstDataRow + i,
            table.countyStateAbbreviatedColumn)
